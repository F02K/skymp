...
import { readPluginsTxt, setPluginEnabled, writePluginsTxt } from './plugins-txt.js';
...
export function getModsSnapshot(runtime, options = {}) {
  ...
  const mergedMods = [...missingPluginEntries, ...mods].sort((left, right) => {
    if (left.attention !== right.attention) {
      return left.attention ? -1 : 1;
    }
    if (left.state !== right.state) {
      const stateOrder = ['enabled', 'disabled', 'inventory_only'];
